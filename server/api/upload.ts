...
export default defineEventHandler(async (event) => {
  ...
  let imageLocation = (await dbInstance.getImageLocation(userId)) ?? "";
  const imageLocations = imageLocation.split(",");
  ...
  try {
    ...
    if (metadata.width && metadata.height) {
      ...
      await dbInstance.upsertImageLocation(userId, imageLocation);
